export const state = () => ({
    ...
    fields:[
        ...
        {key:'count_permission', label:'Jumlah Permissions', sortable:false},
        {key: 'actionrole',label:'', sortable: false, thStyle:{width:'25%'}}, 
    ],
    addLink: '/roles/create',
    btnAction:true,
    edit_data_form: '/roles',
    backBtn:'/roles',
    lists:[],
    permissions:[],
    list:{},
...
export const actions = {
    async get_data({commit, state}){
        ...
        const res = await this.$repositories.role.perPage(params)
    ...
    async save({commit, state}, payload){
        ...
        try{
            const res = await this.$repositories.role.save(payload)
    ...
    async show({commit,state}, id){
        ...
        const res = await this.$repositories.role.show(id)
    ...
    async update({commit, state}, payload){
        ...
        try{
            const res = await this.$repositories.role.update(payload.kode, payload.nama)
    ...
    async allpermission({dispatch, commit}){
        commit('SET_FINISH',1)

        const res = await this.$repositories.permission.all()
